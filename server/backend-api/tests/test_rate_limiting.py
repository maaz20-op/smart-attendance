...
def enable_limiter(db):
    """
    Enable rate limiter for these tests only.
    Also ensures 'db' fixture is active so that app.db.mongo is patched correctly.
    """
...
async def test_verify_otp_rate_limiting():
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        for i in range(3):
            ...
            assert response.status_code in [400, 200, 422, 404]
        ...
        # Verify Retry-After header is present (SlowAPI might not add it by default in test env)
...
async def test_rate_limit_per_ip():
    ...
    # IP2 should still be able to make requests using its own transport/client IP
