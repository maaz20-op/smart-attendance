...
async def test_student_attendance_total_and_percentage_updated_on_confirm(
    ...
    assert attendance["present"] == 1, (
        f"Expected present=1, got {attendance['present']}"
    )
    assert attendance["percentage"] == 100.0, (
        f"Expected percentage=100, got {attendance['percentage']}"
    )
...
async def test_student_attendance_percentage_calculated_correctly_present_and_absent(
    ...
    assert attendance["present"] == 3, (
        f"Expected present=3, got {attendance['present']}"
    )
    assert attendance["absent"] == 2, f"Expected absent=2, got {attendance['absent']}"
    assert attendance["percentage"] == 60.0, (
        f"Expected percentage=60, got {attendance['percentage']}"
    )
...
async def test_get_my_subjects_returns_correct_attendance_data(
    ...
    for i in range(4):
        ...
        await db.subjects.update_one(
            {
                "_id": subject_id,
                "students.student_id": student_user_id,
            },  # Fixed: use userId
            {"$set": {"students.$[s].attendance.lastMarkedAt": "1970-01-01"}},
    ...
    response = await client.get("/students/me/subjects", headers=auth_header)
    ...
    assert subject_data["attendance"] == 80.0, (
        f"Expected 80%, got {subject_data['attendance']}"
    )
    assert subject_data["attended"] == 4, (
        f"Expected 4 attended, got {subject_data['attended']}"
    )
